...
function getEditar(id){
	...
	jQuery.ajax({
		 ...
		 success: function(msg){
			 $(".content").unblock();
			 $("#codigo").val(msg.idlocal);
			 $("#descricaolocal").val(msg.desclocal);
			 
			 $('#secretaria')
	 		    .empty() //empty select
	 		    .append($("<option/>") //add option tag in select
	 		        .val(msg.idsecretaria) //set value for option to post it
	 		        .text(msg.nomsecretaria)) //set a text for show in select
	 		    .val(msg.idsecretaria) //select option of select2
	 		    .trigger("change");			 
			 ...
			 $('#setor')
	 		    .empty() //empty select
	 		    .append($("<option/>") //add option tag in select
	 		        .val(msg.idsetor) //set value for option to post it
	 		        .text(msg.nomsetor)) //set a text for show in select
	 		    .val(msg.idsetor) //select option of select2
	 		    .trigger("change");
		 }
	        ...
	        $('#frm_local').validate({
	            ...
	            submitHandler: function (form) {
					...
					jQuery.ajax({
						 type: "POST",
						 contentType: "application/x-www-form-urlencoded;charset=UTF-8",
						 url: path.val() + "/local/salvar",
						 cache: false,
						 data: {
							 "local.idlocal":jQuery('input[name="codigo"]').val(),
							 "local.desclocal":jQuery('input[name="descricaolocal"]').val().toUpperCase(),
							 "local.setor.idsetor":jQuery('#setor').val()							 
						 },
					   	 beforeSend: function( ) {
					   		loading(".content");
						 },
						 success: function(msg){
							...
							if(!jQuery.isBlank(msg)){
								...
								clear_form_elements("#frm_local");
								
								$('#secretaria').val(null).trigger('change');
