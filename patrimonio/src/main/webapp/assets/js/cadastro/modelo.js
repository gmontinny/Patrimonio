...
function getEditar(id){
	...
	jQuery.ajax({
		 ...
		 success: function(msg){
			 $(".content").unblock();
			 $("#codigo").val(msg.idmodelo);
			 $("#descricaomodelo").val(msg.descmodelo);
			 
			 $('#marca')
	 		    .empty() //empty select
	 		    .append($("<option/>") //add option tag in select
	 		        .val(msg.idmarca) //set value for option to post it
	 		        .text(msg.descmarca)) //set a text for show in select
	 		    .val(msg.idmarca) //select option of select2
	 		    .trigger("change");
		 }
	        ...
	        $('#frm_modelo').validate({
	            ...
	            submitHandler: function (form) {
					...
					jQuery.ajax({
						 type: "POST",
						 contentType: "application/x-www-form-urlencoded;charset=UTF-8",
						 url: path.val() + "/modelo/salvar",
						 cache: false,
						 data: {
							 "modelo.idmodelo":jQuery('input[name="codigo"]').val(),
							 "modelo.descmodelo":jQuery('input[name="descricaomodelo"]').val().toUpperCase(),
							 "modelo.marca.idmarca":jQuery('#marca').val()							 
						 },
					   	 beforeSend: function( ) {
					   		loading(".content");
						 },
						 success: function(msg){
							...
							if(!jQuery.isBlank(msg)){
								...
								clear_form_elements("#frm_modelo");
